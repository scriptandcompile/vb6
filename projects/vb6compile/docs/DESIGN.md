...
fn load_form1() -> vb6runtime::layout::FormHandle {
    ...
    let source = vb6parse::SourceFile::decode_with_replacement("form1.frm", form_file).expect("decode");
    let root = vb6parse::FormFile::parse(&source).expect("parse form1");
    let handle = vb6runtime::layout::load_form(&root.form, vec![], &vb6runtime::layout::LayoutConfig::default());
    handle
}
```
