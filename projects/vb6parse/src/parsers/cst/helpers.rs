...
impl Parser<'_> {
    /// Parse a named clause like `Len = expr` or `Len = expr As mode`.
    ///
    /// Consumes the keyword token, expects `=`, calls `parse_expression()`,
    /// and wraps the expression in an `ExpressionClause` node of the given kind.
    ///
    /// # Arguments
    /// * `keyword` - The expected keyword `SyntaxKind` (e.g. `SyntaxKind::LenKeyword`)
    /// * `as_kind` - Optional `As` keyword `SyntaxKind` for clauses like `As #filenumber`
    /// * `expr_kind` - The `SyntaxKind` to use for the `ExpressionClause` node
    pub(crate) fn parse_named_clause(
        &mut self,
        keyword: SyntaxKind,
        as_kind: Option<SyntaxKind>,
        ...
        if token_kind != keyword {
            self.report_error(ParserError::UnexpectedTokens {
                expected: vec![format!("{:?}", keyword)],
                found: vec![*token],
            });
            return;
        }

        self.builder.start_node(expr_kind.to_raw());
        self.builder.token(token_kind.to_raw(), text);
        self.pos += 1;

        if let Some(as_kw) = as_kind {
            ...
            if let Some((text, token)) = self.tokens.get(self.pos) {
                let token_kind = SyntaxKind::from(*token);
                if token_kind == SyntaxKind::EqualityOperator {
                    self.builder.token(token_kind.to_raw(), text);
                    self.pos += 1;
                } else {
                    self.report_error(ParserError::UnexpectedTokens {
