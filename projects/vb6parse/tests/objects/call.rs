//! Tests for Call statement and procedure call parsing.
//!
//! This module verifies parsing of VB6 procedure invocation:
//! - `Call` - Explicit Call statement with Call keyword
//! - Procedure calls - Implicit procedure calls without Call keyword

#[cfg(test)]
mod tests {
    const SNAPSHOT_PATH: &str = "../../snapshots/parsers/cst/objects/call";

    use vb6parse::ConcreteSyntaxTree;
    use vb6parse::errors::{ErrorKind, ModuleError};
    ...
    fn procedure_call_non_print_semicolon_not_separator() {
        ...
        assert!(
            matches!(
                failure.kind.as_ref(),
                ErrorKind::Module(ModuleError::InvalidSemicolonSeparatorInProcedureCall)
            ),
