...
/// Implementation of the `IsNull` function.
///
/// VB6 behavior:
/// - returns `True` only when `value` is a `Null` variant, `False` for every
///   other value (including `Empty`, which is distinct from `Null`)
/// - never raises an error, regardless of the input value
pub fn is_null(value: &VBVariant) -> VBResult<VBVariant> {
    Ok(VBVariant::from_bool(value.is_null()))
}
...
mod tests {
    ...
    #[test]
    fn returns_true_for_null() {
        assert_eq!(
            is_null(&VBVariant::Null).unwrap(),
    ...
    fn returns_false_for_empty() {
        assert_eq!(
            is_null(&VBVariant::Empty).unwrap(),
    ...
    fn returns_false_for_initialized_values() {
        assert_eq!(
            is_null(&VBVariant::from_byte(0)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_integer(-5)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_long(12345)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_single(1.5)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_double(-2.5)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_currency_scaled(-12_345)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_string("")).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_string("Null")).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_bool(false)).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_date_serial(0.0)).unwrap(),
            VBVariant::from_bool(false)
        );
    }
    ...
    fn returns_false_for_objects_and_arrays() {
        assert_eq!(
            is_null(&VBVariant::Nothing).unwrap(),
            VBVariant::from_bool(false)
        );
        assert_eq!(
            is_null(&VBVariant::from_object(Box::new(TestObject("Test")))).unwrap(),
            VBVariant::from_bool(false)
        );
        let array = VBVariant::array_fixed(VBType::Integer, &[ArrayDimension::new(1, 3)]).unwrap();
        assert_eq!(is_null(&array).unwrap(), VBVariant::from_bool(false));
    }
