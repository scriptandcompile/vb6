...
pub fn log(value: &VBDouble) -> VBResult<VBVariant> {
...
mod tests {
    ...
    fn computes_expected_doubles() {
        let result = log(&VBDouble::from(5.0)).unwrap();
        assert_eq!(result, VBVariant::from_double((5.0_f64).ln()));

        let result = log(&VBDouble::from(12.5)).unwrap();
        assert_eq!(result, VBVariant::from_double((12.5_f64).ln()));
    }

    #[test]
    fn returns_expected_values() {
        let VBVariant::Double(v) = log(&VBDouble::from(0.0)).unwrap() else {
            panic!("expected Double")
        };
        assert_approx_eq(v, (0.0_f64).ln());

        let VBVariant::Double(v) = log(&VBDouble::from(1.0)).unwrap() else {
            panic!("expected Double")
        };
        assert_approx_eq(v, (1.0_f64).ln());

        let VBVariant::Double(v) = log(&VBDouble::from(10.0)).unwrap() else {
            panic!("expected Double")
        };
    ...
    fn rejects_negative_numeric_values() {
        let err = log(&VBDouble::from(-123.0)).unwrap_err();
