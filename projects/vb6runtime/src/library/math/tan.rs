...
pub fn tan(value: &VBDouble) -> VBResult<VBVariant> {
...
mod tests {
    ...
    fn computes_expected_doubles() {
        let result = tan(&VBDouble::from(5.0)).unwrap();
        assert_eq!(result, VBVariant::from_double((5.0_f64).tan()));

        let result = tan(&VBDouble::from(-12.5)).unwrap();
        assert_eq!(result, VBVariant::from_double((-12.5_f64).tan()));
    }

    #[test]
    fn returns_expected_special_angles() {
        let VBVariant::Double(v) = tan(&VBDouble::from(0.0)).unwrap() else {
            panic!("expected Double")
        };
        assert_approx_eq(v, 0.0);

        let VBVariant::Double(v) = tan(&VBDouble::from(1.0)).unwrap() else {
            panic!("expected Double")
        };
