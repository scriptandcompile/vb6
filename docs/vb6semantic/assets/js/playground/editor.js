/**
 * VB6Semantic Playground - Editor Module
 * 
...
function getDefaultCode() {
    return `' VB6Semantic Playground
' Enter your VB6 code here and click Analyze
' The semantic analyzer builds scopes, symbols, and types.
...
Public Sub AddPerson(ByVal name As String, ByVal age As Integer)
    If age < 0 Or age > 150 Then Exit Sub

    Dim idx As Integer
    idx = m_total

    ReDim Preserve m_people(0 To idx)
    m_people(idx).Name = name
    m_people(idx).Age = age

    m_total = m_total + 1
End Sub

Public Function AverageAge() As Double
    Dim total As Double
    Dim i As Integer

    For i = 0 To m_total - 1
        total = total + m_people(i).Age
    Next i

    If m_total > 0 Then
        AverageAge = total / m_total
    End If
End Function
`;
}
...
/**
 * Highlight an entire line in the editor
...
export function highlightLine(line) {
    ...
    highlightRange(targetLine, 1, targetLine, lineLength);
...
export function saveToLocalStorage() {
    if (editor) {
        ...
        localStorage.setItem('vb6semantic-playground-code', content);
        localStorage.setItem('vb6semantic-playground-filetype', currentFileType);
    }
}
...
export function loadFromLocalStorage() {
    const content = localStorage.getItem('vb6semantic-playground-code');
