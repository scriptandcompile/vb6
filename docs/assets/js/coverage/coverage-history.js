// Coverage History Loading and Display
...
const projectConfig = {
    ...
    projectSlug: '',
    dataPath: 'assets/data/vb6parse',
    csvFilename: 'coverage-history.csv',
};

const config = window.ProjectConfig ? { ...projectConfig, ...window.ProjectConfig } : projectConfig;

// GitHub repository URL
const GITHUB_REPO = config.repoUrl;
...
async function loadCoverageWithHistory() {
    try {
        ...
        const response = await fetch(`${config.dataPath}/coverage.json`);
        if (!response.ok) {
        ...
        let history = null;
        try {
            const historyResponse = await fetch(`${config.dataPath}/coverage-history.json`);
    ...
    function renderTable(targetTbody, grouped, sortAscending = true) {
        ...
        sortedDirs.forEach(dir => {
            ...
            group.files.forEach(file => {
                ...
                const coverageFileUrl = `coverage/${filePathWithoutExt}.html`;
